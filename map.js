...
const negateTheBoolean = function (boolean) {
  return !boolean;
};

const negatedBooleansOf = function (booleans) {
  return booleans.map(negateTheBoolean);
};

negatedBooleansOf([true, false, true]);
...
const getDomainNames = function (email) {
  return email.split('@')[1];
};

const domainNamesOf = function (emails) {
  return emails.map(getDomainNames);
};
...
const joineTheArray = function (array) {
  return array.join('');
};

const joinedArraysOf = function (arrayOfArrays) {
  return arrayOfArrays.map(joineTheArray);
};
...
const vowelCount = function (currentCount, element) {
  const vowels = 'aeiou';
...
const getVowelCount = function (string) {
  return [...string].reduce(vowelCount, 0);
};

const countVowelsOf = function (strings) {
  return strings.map(getVowelCount)
};
...
const reversedArraysOf = function (arrays) {
  return arrays.map(getReversedArray);
};

reversedArraysOf([[1, 2, 3], [4, 5, 6]]);

// remove vowels from ["apple", "banana", "grape"] => ["ppl", "bnn", "grp"]

const stringWithoutVowels = function (currentString, element) {
...
const getStringsWithoutVowels = function (string) {
  return [...string].reduce(stringWithoutVowels, '');
};

const withoutVowelsOf = function (strings) {
  return strings.map(getStringsWithoutVowels);
};
...
const cumulativeSum = function (currentArray, element) {
  currentArray.push(+currentArray.slice(-1) + element);
  return currentArray;
};

const getCumulativeSum = function (numbers) {
  return numbers.reduce(cumulativeSum, []);
};
...
cumulativeSumsOf([[1, 2, 3], [4, 5, 6]]);

// reverse words in ["hello world", "goodbye moon"] => ["olleh dlrow", "eybdoog noom"]

const reverseTheWords = function (strings) {
  return strings.split(' ').map(getReversedString).join(' ');
};

const reversedWordsOf = function (strings) {
  return strings.map(reverseTheWords);
};
...
const flattenedArraysOf = function (arrays) {
  return arrays.map(getFlattenedArray);
};

flattenedArraysOf([[1, [2, 3]], [4, [5, 6]]]);
...
const wrappedStringsOf = function (strings) {
  return strings.map(getWrappedWords);
};

wrappedStringsOf(["apple", "banana"]);
...
const getFullName = function (names) {
  return names.firstName + ' ' + names.lastName;
};

const fullNames = function (objects) {
  return objects.map(getFullName);
};

fullNames([{ firstName: "Alice", lastName: "Smith" }, { firstName: "Bob", lastName: "Brown" }]);
...
const firstLetterOfWords = function (word) {
  return word[0];
};

const getAbbreviations = function (details) {
  return details.city.split(' ').map(firstLetterOfWords).join('') + ', ' + details.country;
};

const abbreviations = function (objects) {
  return objects.map(getAbbreviations);
};

abbreviations([{ city: "New York", country: "USA" }, { city: "Los Angeles", country: "USA" }]);
